...
def evaluate_classification(
    ...
    combined_probs = stack_weight * preds_stack + xboost_weight * preds_xgboost  # shape: (n_samples, n_classes)

    # === Decode predicted class index to actual class labels ===
    pred_class_idx = np.argmax(combined_probs, axis=1)  # values: 0 to 4
    index_to_class = dict(zip(range(len(le.classes_)), le.classes_))  # {0: -2, 1: -1, ..., 4: 2}
    true_class_array = np.array([index_to_class[i] for i in pred_class_idx])

    # === Confidence = max probability
    conf_scores = np.max(combined_probs, axis=1)  # shape: (n_samples,)
    ...
    for i, idx in enumerate(X_test_idx):
        ...
        pred_class_idx = np.argmax(combined_probs[i])
        conf = conf_scores[i]
        size = position_sizes[i]
